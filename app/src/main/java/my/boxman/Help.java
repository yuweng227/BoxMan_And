		...
		String s = "";
		 ...
		 if (m_Num == 1) {  // 推关卡界面调用
			 ...
			 s += "\n" +
				 ...
				 "\n" +
				 "【双击--仓管员】，对于某些皮肤，需要手动切换横屏与竖屏，即墙壁或仓管员右旋 90 度；\n" +
				 "【双击--箱子】，在关闭自动箱子编号状态下，进行人工箱子编号（A--Z，A0--A9～Z0--Z9）；\n" +
				 "【双击--墙壁、墙外】，全屏与常规模式转换；\n" +
				 "\n" +
				 "【长按--墙壁】，正推时，提示没有移动过的箱子和没有用到的地板。例外，在关卡初始状态（移动为步数为 0），显示该关卡的答案信息；逆推时，若“逆推时使用正推目标点”开启，则切换是否显示“正逆推双目标点”\n" +
				 "【长按--非点位上的箱子】，提示可以移动的箱子；\n" +
				 "【长按--点位上的箱子或点位（需设置开关）】，正推时，提示关联的网点、网口；\n" +
				 "【长按--空地】，包括逆推时长按点位和正推关闭“长按点位提示关联网”选项时长按点位，提示哪些箱子可以推（拉）到此地，再单击箱子，则自动完成该箱子的推（拉）；\n" +
				 "【长按--仓管员】，正推时，在关卡初态，为切换是否“自动加载最新状态”，否则，为选择“导出 GIF”开始点（从关卡初态开始时，不需要选择）。逆推时，切换“逆推时使用正推目标点”选项开关；\n" +
				 "\n" +
				 "\n" +
				 ...
				 "\n" +
				 "【长按--顶行“关卡序号”】，“死锁提示”开关；\n" +
				 "【长按--顶行“移动步数”】，即景模式与标准模式的切换（即景模式：正逆推互换箱子和目标点）；\n" +
				 "【长按--顶行“推动步数”】，“自动箱子编号”开关；\n" +
				 "【长按--顶行“游标指示”】，“显示标尺”开关；\n" +
				 "\n" +
				 "\n" +
				 "【长按--底行“后退”按钮】，等于“重新开始”；\n" +
				 "【长按--底行“前进”按钮】，逆推，等于“进至尾”（正推时，受“演示时仅推动”选项影响）；\n" +
				 "【长按--底行“旋转”按钮】，等于“0 转”；\n" +
				 "【长按--底行“更多”按钮】，结束游戏，返回上一层。\n" +
				 "\n" +
				 "\n" +
				 "提醒：全屏推关卡时，“前进”和“后退”两个按钮可以拖动位置。";
			 ...
			 s += "\n" +
				 ...
				 "【单指，略延迟滑动--关卡图】，连续绘制。\n" +
				 "\n" +
				 "提醒：编辑区中的“区块”可以单独保存，保存后，回到“创编关卡”浏览界面，能够对该“区块”进行相似查找；进行相似查找时，常规关卡是“标准化”后进行比对的，但对“创编关卡”，则不做“标准化”处理；图片识别送入关卡编辑时，会截取图片分作为关卡编辑的参照底图；双击顶部素材行，可以关闭或打开参照底图的显示。";
			 ...
			 s += "\n" +
					 ...
					 "一般步骤：\n\n" +
					 "  1、边框定位：\n" +
					 "     边框线上带有圆形“指示灯”，粉红色为“点亮”状态，表示该边线被选中，长按可以“点亮”（当没有选择底行的 XSB 元素时，也可以单击“点亮”）。底行的“上下左右”按钮对被选中的边线进行微调定位，也可以在“指示灯”附近双击进行微调；“长按”指示灯（此时，指示灯会闪烁）可以直接拖动当前边线；边框线采用了“黑白”双色线条，对位的最佳效果是以白线“压边”。\n" +
					 "  2、确定横向格子数：\n" +
					 "     当边框确定好后，就可以利用顶行的“增”、“减”按钮调整关卡横向的格子数，而纵向格子数会自动计算。\n" +
					 "  3、识别：\n" +
					 "     当格线调整好之后，先点选底行的 XSB“元素”，然后点击图片中的相应格子，快手会自动启动“识别”。\n" +
					 "  4、识别设置：\n" +
					 "     当识别效果不理想时，可以通过顶行的“度”按钮，调整识别“相似度”，会有一定程度的改善。\n" +
					 "  5、编辑：\n" +
					 "     对于少量的关卡元素，也可以直接手动编辑而不做自动识别。通过点击顶行的“识别”或“编辑”菜单项进行模式切换。在“编辑”模式下，先点选底行的 XSB“元素”，然后点击“格子”即可。特别的，长按底行的 XSB“元素”，会有更多功能，具体不做详述。";
		 } else {    // 主界面调用
		 	s += "====== 特别提醒 ======\n" +
				 "\n" +
				 ...
				 "====== 更新历史 ======\n" +
				 "\n" +
				 "BoxMan9.94：2019-3-29\n" +
				 "1、答案列表加入“时间优先”排序，时间相同时“移动优先”（不愿透露姓名）；\n" +
				 "2、创编关卡的“块选模式”下，双击边界，关卡尺寸自动增加（anian版主）；\n" +
				 "3、进一步完善识别中的编辑功能；\n" +
				 "4、简单优化“帮助”功能；\n" +
				 "5、导出时的 GIF 同时支持“自动箱子编号”和“人工箱子编号”（anian版主）；\n" +
				 "6、优化图像识别中的自定义图片位置功能（anian版主）；\n" +
				 "7、通过改变“标尺”提示框中文字的颜色，提示其可能的“半位”的奇偶性（20603大师）；\n" +
				 "8、其它优化。\n" +
				 "\n" +
				 "BoxMan9.93：2019-3-1\n" +
