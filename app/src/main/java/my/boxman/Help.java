		...
		tv_help = (TextView)this.findViewById(R.id.tvHelp);
		 ...
		 if (m_Num == 1) {  // 推关卡界面调用
			 ...
			 s.append('\n')
				 ...
				 .append('\n')
				 .append("【长按--墙壁】，正推时，提示没有移动过的箱子和没有用到的地板。例外，在关卡初始状态（移动为步数为 0），显示该关卡的答案信息；逆推时，若“逆推时使用正推目标点”开启，则切换是否显示“正逆推双目标点”").append('\n')
				 ...
				 .append("【长按--仓管员】，正推时，在关卡初态，为切换是否“自动加载最新状态”，否则，为选择“导出 GIF”开始点（从关卡初态开始时，不需要选择）。逆推时，切换“逆推时使用正推目标点”选项开关；").append('\n')
				 .append('\n')
				 .append('\n')
