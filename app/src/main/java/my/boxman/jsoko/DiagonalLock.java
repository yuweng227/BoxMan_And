					...
					while (true) {
						...
						if (isPass(current_Position)) {  // 检查点是通道
							...
							if (isGoal(current_Position)) {  // 检查点是目标点
								if (!isTopBottomStarting || isWall_Goal(neighbor1_Position) || isBox(neighbor1_Position-dy) && !isZFreeze(current_Position, neighbor1_Position)) {
...
//							if (!isWall(neighbor2_Position) || !isWall(current_Position + dy)) {
	...
	// 是否处于“四块”冻结
	private boolean isBlock4(int pos) {
		// 左上
		if (isWall_Box(pos + offset[UP]) && isWall_Box(pos + offset[LEFT]) && isWall_Box(pos + offset[UP] + offset[LEFT])) {
			return true;
		}
		// 左下
		if (isWall_Box(pos + offset[DOWN]) && isWall_Box(pos + offset[LEFT]) && isWall_Box(pos + offset[DOWN] + offset[LEFT])) {
			return true;
		}
		// 右上
		if (isWall_Box(pos + offset[UP]) && isWall_Box(pos + offset[RIGHT]) && isWall_Box(pos + offset[UP] + offset[RIGHT])) {
			return true;
		}
		// 右下
		if (isWall_Box(pos + offset[DOWN]) && isWall_Box(pos + offset[RIGHT]) && isWall_Box(pos + offset[DOWN] + offset[RIGHT])) {
			return true;
		}
		return false;
	}
